...
Token scanner(int k){
	  inFilter1.open("filter1.txt");	//opening the file after filtered the comments

	 struct Token tk;
	 string token;
	 int lineNr = 0;			//stores linenumbers in the file
         int count = 0;				//tracks the number of words in the file
	 string line;
	 while(getline(inFilter1, line)){
	      std::istringstream stm(line);     //process each line word by word (separated by white spaces
	      string word;
	      lineNr++;
	      
	      while(stm >> word){		//counting the number of words and if matches with k, what scanner
	         count++;			//is being called with, than that is the next token returned by
		 if(count == k){		//the scanner
	               token = word;
			tk.lineNumber = lineNr;
			break;
		}
	...
	//checking if reached end of file, therefore
	//k which is the number passed to scanner
	//to read the next token should be greater than
	//the number of tokens in the file (count)
	if(k > count && count != 0){
	   tk.lineNumber = lineNr;
	   tk.instance = "EOF_tk";
	   tk.type = EOF_tk;
	...
	//checking if file is empty, if it is program will stop
	if(count == 0){
		cout << "File is empty!\n\n" << endl;
		cout << "Terminating program 2..." << endl;
		exit(0);
	}
...
void displayToken(Token *t){
	   ...
	   if(t->type == ID_tk){
	   	...
	   	cout << "|" << setw(12)  << "EOF_tk " << setw(15) << t->instance << setw(12) << t->lineNumber << endl;
	   }
 }
